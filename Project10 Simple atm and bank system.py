"""
Day 1 Applied - ATM/Bank System with Inheritance & Polymorphism
Refactor of your original Simple ATM: Account is now a base class with
SavingsAccount and CurrentAccount subclasses. The Bank class doesn't care
which subclass it's dealing with - it just calls .withdraw() and the
right behavior happens automatically (that's polymorphism).
"""
...
class Bank:
    ...
    def _row_to_account(self, row) -> Account:
        """Factory: builds the correct subclass based on the stored account_type."""
    ...
    def withdraw(self, account: Account, amount: float):
        """
        Polymorphism in action: this method doesn't know or care whether
        `account` is a SavingsAccount or CurrentAccount. Each subclass's
        own withdraw() enforces its own rules.
        """
    ...
    def transfer(self, sender: Account, receiver_number: str, amount: float):
        ...
        sender.withdraw(amount)   # polymorphic: respects overdraft rules if sender is a CurrentAccount
